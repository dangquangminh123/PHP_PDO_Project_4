...
if(isset($_POST['submit'])){
   ...
   $prev_pass = $fetch_prev_pass['password'];//Mật khẩu trước là mật khẩu trước đây người dùng đã đặt
   $old_pass = sha1($_POST['old_pass']);//Mật khẩu cũ là mật khẩu mà người dùng nhập lại mật khẩu cũ để so sánh với mật khẩu trước
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         //Tình huống người dùng có nhập mật khẩu mới
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `users` SET password = ? WHERE id = ?");
            $update_pass->execute([$confirm_pass, $user_id]);
            $message[] = 'password updated successfully!';
         }//Tình huống này là cả mật khẩu mới và confirm mật khẩu đều để rỗng
         else{
